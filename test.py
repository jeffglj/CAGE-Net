...
def test_process(mode, model, cur_global_step, data_loader, config):
    ...
    with torch.no_grad():
        ...
        # for test_data in tqdm(loader_iter):
            ...
            test_xs = test_data['xs']#[1,1,2000,4]
            test_ys = test_data['ys'].squeeze(-1)#[1,2000]
            num_pts = test_data['xs'].shape[2]#2000
            if num_pts < 4000:
                logits, ys_ds, e_hat, y_hat = model(test_xs, test_ys)#logits   e_hat=[1,9]  y_hat=[1,2000]
                ...
                for batch_idx in range(len(e_hat)):
                    test_xs = test_data['xs'][batch_idx].detach().cpu().numpy()#[1,2000,4]
